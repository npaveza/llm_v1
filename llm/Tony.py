...
class MemoryModule:
    ...
    def retrieve_context(self, current_prompt):
        """Recuperar contexto relevante de la memoria para el prompt actual."""
        relevant_memories = [entry for entry in self.memory if entry["prompt"] in current_prompt]
        context = " ".join([entry["response"] for entry in relevant_memories[-3:]])  # Recuperar las últimas 3 respuestas relevantes
        return context
...
class TonyConversationalModel:
    ...
    def __init__(self, model_path='./Tony', nn_model_path='/home/victhor/Dev/MyModels/tony_nn.pth'):
        ...
        # Inicializar el módulo de memoria
    ...
    def load_neural_network_model(self):
        ...
        state_dict = torch.load(self.nn_model_path)

        # Filtrar claves inesperadas que no están en la definición actual de TonyNN
        filtered_state_dict = {k: v for k, v in state_dict.items() if k in nn_model.state_dict()}

        nn_model.load_state_dict(filtered_state_dict, strict=False)
        nn_model.eval()
        return nn_model
    ...
    def generate_response_with_model(self, prompt):
        ...
        similar_response = self.memory.find_similar_prompt(prompt)
        if similar_response:
        ...
        context_from_memory = self.memory.retrieve_context(prompt)
        ...
        # Agregar la respuesta al historial y a la memoria persistente
        self.conversation_history.append(f"Tony: {enhanced_response}")
        self.memory.add_memory(prompt, enhanced_response)
